...
import { createRetailer, getReps } from "@/lib/data";
import { WARD_META } from "@/lib/data/seed";
import { toaster } from "@/components/toast";
import type { OutletType, RetailerStatus, Tier } from "@/lib/data/types";
...
export default function NewRetailerPage() {
  ...
  const submit = () => {
    ...
    const retailer = createRetailer({
      ...
      zone: wardMeta?.zone ?? "Central",
      address: form.address || `${form.ward}, Nairobi`,
      lat: -1.29,
      lng: 36.82,
      repId: zoneRep?.id ?? reps[0].id,
    });
