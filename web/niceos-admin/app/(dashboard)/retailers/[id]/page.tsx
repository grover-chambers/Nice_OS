...
export default function RetailerProfilePage() {
  ...
  const retailer = useMemo(() => getRetailer(params.id), [params.id]);
  const rep = useMemo(() => (retailer ? getReps().find((r) => r.id === retailer.repId) : undefined), [retailer]);
  const visits = useMemo(() => getVisits({ retailerId: params.id }), [params.id]);
  const obs = useMemo(() => getCompetitorObservations().filter((o) => o.retailerId === params.id), [params.id]);
