...
export default function RoutePlannerPage() {
  ...
  const rep = getReps().find((r) => r.id === route?.repId);
  ...
  const save = () => {
    replaceRouteStops(params.id, stops);
    reload();
      ...
      <PageHeader
        ...
        actions={
          <div className="flex flex-wrap items-center gap-2">
            <button
              onClick={() => act(() => optimizeRoute(params.id), "Route optimised — stops re-sequenced for the shortest path")}
            ...
            {route.status === "draft" && (
              <button
                onClick={() => act(() => setRouteStatus(params.id, "submitted"), "Submitted for approval")}
            ...
            {(route.status === "approved" || isReviewable) && (
              <button
                onClick={() => act(() => setRouteStatus(params.id, "approved"), "Route approved")}
            ...
            {isReviewable && (
              <button
                onClick={() => {
                  ...
                  if (reason) act(() => setRouteStatus(params.id, "needs-revision", reason), "Revision requested");
                }}
      ...
      <div className="grid grid-cols-1 gap-4 xl:grid-cols-5">
        ...
        <Card title="Route map" subtitle="Sequenced route across the territory" className="xl:col-span-3" pad={false}>
          <MapViewWrapper
            ...
            route={{ id: params.id, line: routeLine, stops: stops.map((s, i) => {
