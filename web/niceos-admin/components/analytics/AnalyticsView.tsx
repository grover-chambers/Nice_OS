...
export default function AnalyticsView({
  ...
  visits: Visit[];
  zoneCoverage: { zone: string; coveragePct: number }[];
}) {
  ...
  const shelfHealth: Record<string, { name: string; full: number; low: number; out: number; availability: number }> = {};
  ...
  for (const [sku, r] of Object.entries(acc)) {
    const total = r.full + r.low + r.out;
    shelfHealth[sku] = { ...r, availability: total ? Math.round((r.full / total) * 100) : 0 };
  }
  const shelfList = Object.values(shelfHealth);
